...
func TestNoHostAgentIsStillARefusalNamingWhatIsMissing(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// Addressable on purpose. A tier-3 action with no devpath is refused
		// earlier, and for a different missing thing — the position, not the
		// agent — which is a refusal this case is not about.
		{"tier 3", Action{Tier: 3, TierName: "usb_reset", HostID: "h01",
			Devpath: "usb:3-1.4", RackSlot: "r1u3s04"}, "USBDEVFS_RESET"},
