...
func TestFenceClientIsOffUntilAllThreeFilesAreSet(t *testing.T) {
	...
	if cfg.Fence.Enabled() || cfg.Fence.TLS != nil {
...
//
// Falsify: stop assigning cfg.Fence.TLS in Load.
func TestFenceClientLoadsAndDescribesTheCertificate(t *testing.T) {
	...
	if len(cfg.Fence.TLS.Certificates) != 1 || cfg.Fence.TLS.RootCAs == nil {
		t.Fatalf("the TLS configuration must present one certificate and trust the CA: %+v", cfg.Fence.TLS)
	}
	if cfg.Fence.TLS.MinVersion < 0x0304 {
		t.Errorf("MinVersion = %#x, want TLS 1.3; the proxy serves nothing older", cfg.Fence.TLS.MinVersion)
	}
	s := cfg.Summary()
