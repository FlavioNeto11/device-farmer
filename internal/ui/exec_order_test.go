...
//
// The catalogue moved into a dialog first, behind one button. This file is
// about the other half — the pre-flight, which is reference material too, and
// which reads as a wall in exactly the same way when it sits above the button
// it explains.
//
...
func TestTheRunButtonComesBeforeThePreflight(t *testing.T) {
	...
	//
	// The class is matched as a PREFIX, without its closing brace, because the
	// builder appends ' cmd-compact' to it for the bulk form. Requiring the
	// whole literal would make this half of the test blind the moment a
	// modifier class is added, which is the failure mode this comment exists to
...
// TestTheHostOwnsTheRunControl is the other half of the same decision, and it
// is here because the first test cannot see it.
//
// The builder owns the ORDER — command, wire, consent, Run, pre-flight — and
// the host owns the Run control itself. That split is why the drawer can put
// the force-and-reason consent ABOVE its Run button, which is the only place a
// consent can go: revealed below the button that needs it, it is a field an
// operator finds by being refused. A builder that rendered its own Run button
// would have to guess that order for every host, and the bulk form, which
// submits through the host <form>'s own button, would then have two.
//
// Falsify: build a Run button inside commandBuilder and drop the two slots.
func TestTheHostOwnsTheRunControl(t *testing.T) {
	src := execSource(t)

	for _, slot := range []string{"runSlot", "consentSlot"} {
		if strings.Count(src, slot) < 3 {
			t.Errorf("assets/exec.js names %s fewer than three times: it has to be declared, "+
				"returned to the host and filled by it. The builder is then owning a control "+
				"only the host can place.", slot)
		}
	}

	at := strings.Index(src, "builder.consentSlot.append(")
	runAt := strings.Index(src, "builder.runSlot.append(")
	if at < 0 || runAt < 0 {
		t.Fatal("assets/exec.js's execPanel no longer fills the builder's two slots; the panel " +
			"changed shape and this test is now blind")
	}

	// The consent goes into the slot ABOVE the run row. That is a property of
	// the tree, so it is checked there: consentSlot must be mounted before
	// runRow in the composition.
	open := strings.Index(src, "const node = el('div', { class: 'cmd-builder'")
	if open < 0 {
		t.Fatal("assets/exec.js no longer composes a cmd-builder node; this test is now blind")
	}
	...
	if c, r := strings.Index(tree, "consentSlot"), strings.Index(tree, "runRow"); c < 0 || r < 0 || c > r {
		t.Errorf("the cmd-builder tree mounts consentSlot at %d and runRow at %d.\n\n"+
			"The force-and-reason step must sit above the button it gates. Below it, an "+
			"operator meets it by being refused, which is the silence unit 7 removed.",
			c, r)
	}
}
