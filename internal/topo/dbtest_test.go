...
//     all. The parser, the hub filter and the planner are exercised against
//     fixture trees everywhere.
//
//   - The database is created fresh per run and dropped afterwards, and the
//     schema is emptied between tests. Discovery reconciles a scan against
//     every slot the host already has, so a hub left behind by one test would
//     be "the rack" the next test measures its retirements against.
...
func runSuite(m *testing.M) (code int) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "topo tests: open DATABASE_URL: %v\n", err)
		return 1
	}
	defer admin.Close()
	...
	if err := admin.PingContext(ctx); err != nil {
		fmt.Fprintf(os.Stderr, "topo tests: connect to DATABASE_URL: %v\n", err)
		return 1
	}

	name := scratchName("df_topo_test")
	dsn, err := dsnForDatabase(base, name)
	if err != nil {
		fmt.Fprintf(os.Stderr, "topo tests: %v\n", err)
		return 1
	}

	if _, err := admin.ExecContext(ctx, `SELECT pg_advisory_lock($1)`, setupLockKey); err != nil {
		fmt.Fprintf(os.Stderr, "topo tests: take setup lock: %v\n", err)
		return 1
	}
	if _, err := admin.ExecContext(ctx, `CREATE DATABASE `+quoteIdent(name)); err != nil {
		...
		fmt.Fprintf(os.Stderr, "topo tests: create scratch database (the role needs CREATEDB): %v\n", err)
		return 1
	}
	defer func() {
		if testPool != nil {
			...
			case <-time.After(15 * time.Second):
				fmt.Fprintln(os.Stderr,
					"topo tests: a connection was still checked out at teardown; forcing the drop")
			}
		}
...
func openTestPool(ctx context.Context, dsn string) (*pgxpool.Pool, error) {
	...
	// A pass registers hub by hub in its own transaction while the test reads
	// alongside; a few connections are plenty and the DB-backed tests do not
...
func requireDB(t *testing.T) *pgxpool.Pool {
	t.Helper()
	if testPool == nil {
		t.Skip("DATABASE_URL is not set; skipping the SQL-backed topo tests")
	}
	return testPool
}
