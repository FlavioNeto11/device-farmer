...
//
//   - The database is created fresh per run and dropped afterwards, and
//     resetSchema runs between tests. The scheduler polls the WHOLE queue and
//     allocates from the whole farm; a job or a free device left behind by one
//     test is a candidate the next test never asked for, and a placement it
//     cannot explain.
//
// It is deliberately the same shape as internal/reaper's harness. Two loops
// that share a database and a set of invariants should be testable by the same
// moves, or the second one gets tested less carefully than the first.
...
func runSuite(m *testing.M) (code int) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "scheduler tests: open DATABASE_URL: %v\n", err)
		return 1
	}
	defer admin.Close()
	...
	if err := admin.PingContext(ctx); err != nil {
		fmt.Fprintf(os.Stderr, "scheduler tests: connect to DATABASE_URL: %v\n", err)
		return 1
	}

	name := scratchName("df_scheduler_test")
	dsn, err := dsnForDatabase(base, name)
	if err != nil {
		fmt.Fprintf(os.Stderr, "scheduler tests: %v\n", err)
		return 1
	}

	if _, err := admin.ExecContext(ctx, `SELECT pg_advisory_lock($1)`, setupLockKey); err != nil {
		fmt.Fprintf(os.Stderr, "scheduler tests: take setup lock: %v\n", err)
		return 1
	}
	if _, err := admin.ExecContext(ctx, `CREATE DATABASE `+quoteIdent(name)); err != nil {
		...
		fmt.Fprintf(os.Stderr, "scheduler tests: create scratch database (the role needs CREATEDB): %v\n", err)
		return 1
	}
	defer func() {
		...
		if testPool != nil {
			...
			case <-time.After(15 * time.Second):
				fmt.Fprintln(os.Stderr,
					"scheduler tests: a connection was still checked out at teardown; forcing the drop")
			}
		}
		...
		if _, err := admin.ExecContext(dctx,
			...
			fmt.Fprintf(os.Stderr, "scheduler tests: drop scratch database %s: %v\n", name, err)
		}
	}()

	migrateErr := migrateScratch(ctx, dsn)
	if _, err := admin.ExecContext(ctx, `SELECT pg_advisory_unlock($1)`, setupLockKey); err != nil {
		fmt.Fprintf(os.Stderr, "scheduler tests: release setup lock: %v\n", err)
	}
	if migrateErr != nil {
		fmt.Fprintf(os.Stderr, "scheduler tests: %v\n", migrateErr)
		return 1
...
func openTestPool(ctx context.Context, dsn string) (*pgxpool.Pool, error) {
	...
	// run two schedulers at once, so the pool must comfortably exceed the
	// number of leadership connections in flight or the tests deadlock on a
	// property they are supposed to be measuring.
	pc.MaxConns = 12
...
func requireDB(t *testing.T) *pgxpool.Pool {
	t.Helper()
	if testPool == nil {
		t.Skip("DATABASE_URL is not set; skipping the SQL-backed scheduler tests")
	}
	return testPool
}
...
// those four subsystems, and reports their failures as scheduler failures.
// ---------------------------------------------------------------------------
...
// newPool adds another allocation pool. Pools are the scheduler's coarsest
// partition: farm.lease_acquire will not cross one.
func (f *fixture) newPool(id string) string {
	f.t.Helper()
	f.exec(`INSERT INTO farm.pools (id) VALUES ($1)`, id)
	return id
}

// newTenant adds a tenant with a device cap. maxDevices 0 means unlimited.
func (f *fixture) newTenant(id string, maxDevices int) string {
	f.t.Helper()
	f.exec(`INSERT INTO farm.tenants (id, name, max_devices) VALUES ($1, $1, $2)`, id, maxDevices)
	return id
}

// newQueue adds a queue. priority is ASCENDING urgency in the scheduler's poll
// order, and maxDevices 0 means unlimited.
func (f *fixture) newQueue(id, tenantID string, priority, maxDevices int) string {
	f.t.Helper()
	f.exec(`INSERT INTO farm.queues (id, tenant_id, priority, max_devices) VALUES ($1, $2, $3, $4)`,
		id, tenantID, priority, maxDevices)
	return id
}
...
// liveLeases counts the leases that currently hold a device, farm-wide.
//
// Most scheduler tests assert on this number rather than on a particular row:
// the failure that matters is not "the wrong lease exists", it is "a lease
// exists at all when nothing should have been placed".
func (f *fixture) liveLeases() int {
	f.t.Helper()
	var n int
	f.scan(&n, `SELECT count(*) FROM farm.leases WHERE state IN ('held','suspect')`)
	return n
}

// leaseFor returns the live lease of one job, or ok=false when it has none.
func (f *fixture) leaseFor(jobID string) (id string, device string, fence int64, ok bool) {
	f.t.Helper()
	err := f.pool.QueryRow(f.ctx, `
SELECT id::text, device_id::text, fence
  FROM farm.leases
 WHERE job_id = $1::uuid AND state IN ('held','suspect')`, jobID).Scan(&id, &device, &fence)
	if err != nil {
		return "", "", 0, false
	}
	return id, device, fence, true
}

// jobState reads farm.jobs.state.
func (f *fixture) jobState(jobID string) string {
	f.t.Helper()
	var s string
	f.scan(&s, `SELECT state FROM farm.jobs WHERE id = $1::uuid`, jobID)
	return s
}
...
// newScheduler builds a Scheduler wired to the scratch pool with a lock key
// derived from the test name, so two tests can never fight over leadership.
//
// Each call mints its own HolderInstance, exactly as a separate process would:
// a shared instance would make two "replicas" indistinguishable to
// farm.lease_renew and quietly weaken every leader-election test below.
//
...
func (f *fixture) newScheduler(rec *logRecorder, holder string) *Scheduler {
	f.t.Helper()
	...
	s, err := New(Config{
		...
		Holder:         holder,
		HolderInstance: instance,
		LockKey:        lockKeyFor(f.t.Name()),
		Logger:         rec.logger(),
	})
	if err != nil {
		f.t.Fatalf("new scheduler: %v", err)
	}
	f.t.Cleanup(func() { s.lead.release(context.Background()) })
	return s
}
